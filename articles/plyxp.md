...
Some of the optimized routines in `plyxp` for operating on objects are
called by some of the functions implemented in
*tidySummarizedExperiment*. `plyxp` and `tidySummarizedExperiment` both
provide *dplyr*-style interfaces to *SummarizedExperiment* objects, and
can easily be loaded and used in the same R session without the
functions conflicting. By casting objects with the `new_plyxp`
constructor, users can enable the `plyxp`-driven functionality.
...
```
    ...
    ##  plyxp                * 1.7.1   2026-05-06 [1] Bioconductor
